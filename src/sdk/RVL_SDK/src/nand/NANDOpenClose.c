...
s32 nandSafeOpenAsync(const char* path, NANDFileInfo* info, const u8 accType, void* buf, const u32 length, NANDCallback cb, NANDCommandBlock* block, BOOL privilege_flag, BOOL simple_flag) {
    ...
    if (accType == 1) {
        ...
        block->fileInfo = info;
        block->callback = cb;
        fd = ISFS_OpenAsync(info->origPath, 1, nandReadOpenCallback, block);
        ...
        ret = ISFS_CreateDirAsync("/tmp/sys", 0, 3, 3, 3, nandSafeOpenCallback, block);
...
void nandSafeOpenCallback(const ISFSError result, void* ctxt) {
    ...
    if ((result >= 0) || ((result == ISFS_ERROR_EXISTS) && b->state == 0)) {
        ...
        if (b->state == 1) {
            ret = ISFS_GetAttrAsync(info->origPath, &b->ownerId, &b->groupId, &b->attr, &b->ownerAcc, &b->groupAcc, &b->othersAcc, nandSafeOpenCallback, ctxt);
        }
        else if (b->state == 2) {
            ret = ISFS_OpenAsync(info->origPath, 1, nandSafeOpenCallback, ctxt);
        }
        else if (b->state == 3) {
            char tmpDir[64];
            b->uniqNo = nandGetUniqueNumber();
            sprintf(tmpDir, "%s/%08x", "/tmp/sys", b->uniqNo);
            ret = ISFS_CreateDirAsync(tmpDir, 0, 3, 0, 0, nandSafeOpenCallback, ctxt);
        }
        else if (b->state == 4) {
            char filename[13];
            ...
            ret = ISFS_CreateFileAsync(info->tmpPath, b->attr, b->ownerAcc, b->groupAcc, b->othersAcc, nandSafeOpenCallback, ctxt);
        }
        else if (b->state == 5) {
            info->stage = 4;

            if (info->accType == 2) {
                ret = ISFS_OpenAsync(info->tmpPath, 2, nandSafeOpenCallback, ctxt);
            }
            else if (info->accType == 3) {
                ret = ISFS_OpenAsync(info->tmpPath, 3, nandSafeOpenCallback, ctxt);
            }
            else {
...
void nandSafeCloseCallback(ISFSError result, void* ctxt) {
    ...
    if (result == 0) {
        ...
        if (b->state == 11) {
            ...
            info->stage = 7;
            ret = ISFS_RenameAsync(info->tmpPath, info->origPath, nandSafeCloseCallback, ctxt);
        }
        else if (b->state == 13) {
            char tmpdir[64] = "";
            nandGetParentDirectory(tmpdir, info->tmpPath);
            ret = ISFS_DeleteAsync(tmpdir, nandSafeCloseCallback, ctxt);
        }
        else if (b->state == 14) {
            if (!(b->simpleFlag)) {
                info->stage = 9;
