...
#define DEV_IS_NOT_WBC(dev_)                                                             \
    (!WUDIsLinkedWBC()                                                                   \
     || (WUDIsLinkedWBC() && WUD_DEV_NAME_IS_CNT((dev_)->small.devName)))
...
extern void
bta_hh_co_open(UINT8 dev_handle, UINT8 sub_class, UINT16 attr_mask, UINT8 app_id);
extern void bta_hh_co_close(UINT8 dev_handle, UINT8 app_id);
extern BOOLEAN
bta_dm_co_get_compress_memory(tBTA_SYS_ID id, UINT8 **memory_p, UINT32 *memory_size);
        ...
        for (i = 0; i < p_hh_e15->at_0x04; i++) {
            if (p_hh_e15->at_0x06[i].handle < WUD_MAX_DEV_ENTRY) {
                WUDiSetQueueSizeForHandle(
                    p_hh_e15->at_0x06[i].handle, p_hh_e15->at_0x06[i].queueSize
                );
                WUDiSetNotAckNumForHandle(
                    p_hh_e15->at_0x06[i].handle, p_hh_e15->at_0x06[i].notAckNum
                );
            }
        }
...
extern void bta_hh_co_open(
    UINT8 dev_handle __attribute__((unused)),
    UINT8 sub_class __attribute__((unused)),
...
extern void bta_hh_co_close(
    UINT8 dev_handle __attribute__((unused)), UINT8 app_id __attribute__((unused))
) {
    return;
}

extern BOOLEAN bta_dm_co_get_compress_memory(
    tBTA_SYS_ID id __attribute__((unused)),
