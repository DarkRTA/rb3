...
static BOOL
__wpadCheckCalibration(byte_t *data, u8 checksumOffset, ChecksumType checksumType);
static void __wpadGetFsConfig(WPADChannel chan, byte_t *config);
static void __wpadGetClConfig(WPADChannel chan, byte_t *config);
static void __wpadGetMpConfig(WPADChannel chan, byte_t *config, MplsConfigDataType type);
...
static void __parse_dpd_data(
    WPADChannel chan, WPADStatus **status, u8 dpdCommand, byte_t *data, u8 length
);
static void __parse_dpdex_data(
    WPADChannel chan, WPADStatusEx **statusEx, u8 objIndex, byte_t *data, u8 length
);
static void __parse_acc_data(
    WPADChannel chan, WPADStatus **status, u8 accFormat, byte_t *data, u8 length
);
static s16 __clamp(s16 x, s16 low, s16 high);
static void __parse_fs_data(
    WPADChannel chan, WPADFSStatus **fsStatus, u8 fsFormat, byte_t *data, u8 length
);
static void __parse_cl_data(
    WPADChannel chan, WPADCLStatus **clStatus, u8 clFormat, byte_t *data, u8 length
);
static void __parse_bk_data(
    WPADChannel chan, WPADBKStatus **bkStatus, u8 bkFormat, byte_t *data, u8 length
);
static void __parse_tr_data(
    WPADChannel chan, WPADTRStatus **trStatus, u8 trFormat, byte_t *data, u8 length
);
static void __parse_bl_data(
    WPADChannel chan, WPADBLStatus **blStatus, u8 blFormat, byte_t *data, u8 length
);
static void __parse_vs_data(
    WPADChannel chan, WPADVSStatus **vsStatus, u8 vsFormat, byte_t *data, u8 length
);
static void __parse_mp_data(
    WPADChannel chan, WPADMPStatus **mpStatus, u8 mpFormat, byte_t *data, u8 length
);
static void
__parse_ext_data(WPADChannel chan, void *parseBuf, u8 dataFormat, byte_t *data, u8 length);
...
static void __wpadAbortInitExtension(WPADChannel chan, WPADResult result) {
    ...
    if (result == WPAD_ENODEV) {
        ...
        WPADiSendSetReportType(
            &p_wpd->extCmdQueue, p_wpd->dataFormat, p_wpd->disableContReport, NULL
        );
        return;
    }
...
static void __wpadGetExtConfig(WPADChannel chan, WPADResult result) {
    ...
    if (p_wpd->devType == WPAD_DEV_FS) {
        ...
        p_wpd->extConfig.mpls.high.pitchZero = p_wpd->extConfig.mpls.high.yawZero =
            p_wpd->extConfig.mpls.high.rollZero = 8192.0f;
        p_wpd->extConfig.mpls.high.pitchScale = p_wpd->extConfig.mpls.high.yawScale =
            p_wpd->extConfig.mpls.high.rollScale = 5333.0f;
        p_wpd->extConfig.mpls.high.degrees = 1200;

        p_wpd->extConfig.mpls.low.pitchZero = p_wpd->extConfig.mpls.low.yawZero =
            p_wpd->extConfig.mpls.low.rollZero = 8192.0f;
        p_wpd->extConfig.mpls.low.pitchScale = p_wpd->extConfig.mpls.low.yawScale =
...
static void __wpadGetExtType(WPADChannel chan, WPADResult result) {
    ...
    if (result == WPAD_ESUCCESS) {
        ...
        if (memcmp(data, _cExtInvalidData, 16) == 0 || (data[12] == 0 && data[13] == 0)) {
            ...
            case 16:
                _wpadDevType[chan] = WPADIsEnabledTRN() ? WPAD_DEV_TRAIN : WPAD_DEV_251;
                break;

            case 17:
                _wpadDevType[chan] = WPADIsEnabledTKO() ? WPAD_DEV_TAIKO : WPAD_DEV_251;
                break;
            ...
            case 3:
                if (__OSInIPL) {
                    ...
                    case 3:
                        _wpadDevType[chan] =
                            WPADIsEnabledTBL() ? WPAD_DEV_TURNTABLE : WPAD_DEV_251;
                        break;
        ...
        if (_wpadDevType[chan] == WPAD_DEV_251 || _wpadDevType[chan] == WPAD_DEV_252) {
            p_wpd->devType = _wpadDevType[chan];
            p_wpd->devMode = _wpadDevMode[chan];
            ...
            if (_wpadDevType[chan] == WPAD_DEV_MOTION_PLUS
                ...
                // see
                // https://wiibrew.org/wiki/Wiimote/Extension_Controllers#Encryption_setup
                WPADiSendWriteDataCmd(
                    queue, 0xaa, WM_REG_EXTENSION_F0, &__wpadAbortInitExtension
                );
                WPADiSendWriteData(
                    queue,
                    &p_wpd->encryptionKey[0],
                    6,
...
static void __wpadInitExtension(WPADChannel chan) {
    ...
    if (p_wpd->isInitingMpls) {
        WPADiSendWriteDataCmd(
            &p_wpd->extCmdQueue, cmd, WM_REG_EXTENSION_FB, &__wpadAbortInitExtension
        );
        WPADiSendReadData(
            &p_wpd->extCmdQueue,
            p_wpd->wmReadDataBuf,
        ...
        WPADiSendWriteDataCmd(
            &p_wpd->extCmdQueue, 0x55, WM_REG_EXTENSION_F0, &__wpadAbortInitExtension
        );
        WPADiSendWriteDataCmd(
            &p_wpd->extCmdQueue, cmd, WM_REG_EXTENSION_FB, &__wpadAbortInitExtension
        );
        WPADiSendReadData(
            &p_wpd->extCmdQueue,
            &p_wpd->wmReadDataBuf[10],
            16 - 10,
            WM_REG_EXTENSION_F0 + 10,
            &__wpadAbortInitExtension
        );
    }
...
signed WPADiHIDParser(UINT8 port, UINT8 *p_rpt) {
    ...
    byte_t *rxBuf;
    if (p_rpt[RPT_REPORT_ID] >= RPTID_IN_LO && p_rpt[RPT_REPORT_ID] <= RPTID_IN_HI) {
...
static void __a1_21_user_data(u8 chan, byte_t *hidReport, void *rxBuffer) {
    ...
    WPADResult cbRet;
    if ((u16)addrLow >= readAddrLow && addrLow <= readAddrLow + p_wpd->wmReadLength) {
        if (errCode == 0) {
            memcpy(p_wpd->wmReadDataPtr + addrDiff, &hidReport[RPT21_DATA], size);
        }
        ...
        if (readAddrLow + p_wpd->wmReadLength == addrLow + size) {
            ...
            if (readAddrHigh == 0x04a4
                && (p_wpd->extState == WPAD_STATE_EXT_ENCRYPTED
                    || p_wpd->extState == WPAD_STATE_EXT_ENCRYPTED_3RD)) {
                WPADiDecode(chan, p_wpd->wmReadDataPtr, p_wpd->wmReadLength, readAddrLow);
            }

            if ((p_wpd->wmReadAddress == WM_ADDR_MEM_DEV_CONFIG_0
                 && p_wpd->configIndex == 0)
                || (p_wpd->wmReadAddress == WM_ADDR_MEM_176C && p_wpd->configIndex == 1
                )) {
...
static void __parse_dpd_data(
    WPADChannel chan, WPADStatus **status, u8 dpdFormat, byte_t *data, u8 length
) {
    wpad_cb_st *p_wpd = __rvl_p_wpadcb[chan];
    ...
    for (i = 0; i < WPAD_MAX_DPD_OBJECTS; i++) {
        if (status[0]->obj[i].x != 0 || status[0]->obj[i].y != DPD_MAX_Y) {
            fx = (f32)status[0]->obj[i].x + _wpadCalibrationX[chan] - _wpadCenterX[chan];
            fy = (f32)status[0]->obj[i].y + _wpadCalibrationY[chan] - _wpadCenterY[chan];
...
static void __parse_dpdex_data(
    ...
    if (statusEx[0]->obj[objIndex].size == 0 || statusEx[0]->obj[objIndex].x == DPD_MAX_X
...
static void __parse_fs_data(
    WPADChannel chan,
    WPADFSStatus **fsStatus,
    u8 fsFormat __attribute__((unused)),
    byte_t *data,
    u8 length __attribute__((unused))
) {
    wpad_cb_st *p_wpd = __rvl_p_wpadcb[chan];
...
static void __parse_cl_data(
    ...
    case WPAD_DEV_MODE_CLASSIC_REDUCED:
        ...
        clStatus[0]->clRStickX = MAKE_CL_REDUCED_STICK_RX(data[0], data[1], data[2]);
...
static void __parse_bk_data(
    WPADChannel chan,
    WPADBKStatus **bkStatus,
    u8 bkFormat __attribute__((unused)),
    byte_t *data,
    u8 length
) {
    wpad_cb_st *p_wpd __attribute__((unused)) = __rvl_p_wpadcb[chan];
...
static void __parse_tr_data(
    WPADChannel chan __attribute__((unused)),
    WPADTRStatus **trStatus,
    u8 trFormat __attribute__((unused)),
    byte_t *data,
    u8 length __attribute__((unused))
) {
...
static void __parse_bl_data(
    WPADChannel chan,
    WPADBLStatus **blStatus,
    u8 blFormat __attribute__((unused)),
    byte_t *data,
    u8 length __attribute__((unused))
) {
    wpad_cb_st *p_wpd = __rvl_p_wpadcb[chan];
...
static void __parse_vs_data(
    WPADChannel chan,
    WPADVSStatus **vsStatus,
    u8 vsFormat __attribute__((unused)),
    byte_t *data,
    u8 length __attribute__((unused))
) {
    /*
...
static void __parse_mp_data(
            ...
            if ((data[0] == 0xff && data[1] == 0xff) || p_wpd->parseMPState != 5
                ...
                if (p_wpd->devType == WPAD_DEV_MPLS_PT_FS) {
                    ...
                    if (!p_wpd->calibrated) {
                        ...
                        p_wpd->extConfig.fs.stickXCenter = mpStatus[0]->ext.fs.fsStickX;
                        p_wpd->extConfig.fs.stickYCenter = mpStatus[0]->ext.fs.fsStickY;
                    }
    ...
    if (p_wpd->parseMPState == 2 || p_wpd->parseMPState == 9) {
        if (WPADiSendReadData(
                &p_wpd->extCmdQueue, p_wpd->wmReadDataBuf, 16, WM_REG_EXTENSION_40, NULL
            )) {
            p_wpd->parseMPState++;
        }
        ...
        if (WPADiIsAvailableCmdQueue(&p_wpd->extCmdQueue, 2)) {
            WPADiSendWriteDataCmd(&p_wpd->extCmdQueue, 0x01, WM_REG_EXTENSION_F3, NULL);
            WPADiSendReadData(
                &p_wpd->extCmdQueue, &p_wpd->parseMPBuf, 1, WM_REG_EXTENSION_F3, NULL
            );
...
static void __parse_ext_data(
    WPADChannel chan,
    void *parseBuf,
    u8 extFormat __attribute__((unused)),
    byte_t *data,
    u8 length
) {
    wpad_cb_st *p_wpd = __rvl_p_wpadcb[chan];
    ...
    if (p_wpd->extErr == WPAD_ESUCCESS) {
        ...
        if (p_wpd->devType == WPAD_DEV_FS) {
            __parse_fs_data(
                chan,
                (WPADFSStatus **)&parseBuf,
                p_wpd->devMode,
                p_wpd->extDataBuf,
                p_wpd->extDataLength
            );
        } else if (p_wpd->devType == WPAD_DEV_CLASSIC || p_wpd->devType == WPAD_DEV_GUITAR
...

        {
            __parse_cl_data(
                chan,
                (WPADCLStatus **)&parseBuf,
                p_wpd->devMode,
                p_wpd->extDataBuf,
                p_wpd->extDataLength
            );
        } else if (p_wpd->devType == WPAD_DEV_BULK_1 || p_wpd->devType == WPAD_DEV_BULK_2
...
static void __a1_31_data_type(u8 chan, byte_t *hidReport, void *rxBuffer) {
    ...
    __parse_btn_data(p_wpd, status, BTN_FORMAT_STANDARD, hidReport, 0);
    __parse_acc_data(chan, &status, ACC_FORMAT_STANDARD, &hidReport[RPT_ACC_OFFSET], 0);
}
...
static void __a1_32_data_type(u8 chan, byte_t *hidReport, void *rxBuffer) {
    ...
    __parse_btn_data(p_wpd, status, BTN_FORMAT_STANDARD, hidReport, 0);
    __parse_ext_data(
        chan, rxBuffer, p_wpd->devMode, &hidReport[RPT32_EXT_OFFSET], RPT32_EXT_LENGTH
    );
}
...
static void __a1_33_data_type(u8 chan, byte_t *hidReport, void *rxBuffer) {
    ...
    __parse_dpd_data(
        chan,
        &status,
        p_wpd->currentDpdCommand,
        &hidReport[RPT33_DPD_OFFSET],
        RPT33_DPD_LENGTH
    );
}
...
static void __a1_34_data_type(u8 chan, byte_t *hidReport, void *rxBuffer) {
    ...
    __parse_btn_data(p_wpd, status, BTN_FORMAT_STANDARD, hidReport, 0);
    __parse_ext_data(
        chan, rxBuffer, p_wpd->devMode, &hidReport[RPT34_EXT_OFFSET], RPT34_EXT_LENGTH
    );
}
...
static void __a1_35_data_type(u8 chan, byte_t *hidReport, void *rxBuffer) {
    ...
    __parse_acc_data(chan, &status, ACC_FORMAT_STANDARD, &hidReport[RPT_ACC_OFFSET], 0);
    __parse_ext_data(
        chan, rxBuffer, p_wpd->devMode, &hidReport[RPT35_EXT_OFFSET], RPT35_EXT_LENGTH
    );
}
...
static void __a1_36_data_type(u8 chan, byte_t *hidReport, void *rxBuffer) {
    ...
    __parse_dpd_data(
        ...
        &status,
        p_wpd->currentDpdCommand,
        &hidReport[RPT36_DPD_OFFSET],
        RPT36_DPD_LENGTH
    );
    __parse_ext_data(
        chan, rxBuffer, p_wpd->devMode, &hidReport[RPT36_EXT_OFFSET], RPT36_EXT_LENGTH
    );
}
...
static void __a1_37_data_type(u8 chan, byte_t *hidReport, void *rxBuffer) {
    ...
    __parse_dpd_data(
        ...
        &status,
        p_wpd->currentDpdCommand,
        &hidReport[RPT37_DPD_OFFSET],
        RPT37_DPD_LENGTH
    );
    __parse_ext_data(
        chan, rxBuffer, p_wpd->devMode, &hidReport[RPT37_EXT_OFFSET], RPT37_EXT_LENGTH
    );
}
...
static void __a1_3d_data_type(u8 chan, byte_t *hidReport, void *rxBuffer) {
    ...
    __parse_ext_data(
        chan, rxBuffer, p_wpd->devMode, &hidReport[RPT3D_EXT_OFFSET], RPT3D_EXT_LENGTH
    );
}
...
static void __a1_3e_data_type(u8 chan, byte_t *hidReport, void *rxBuffer) {
    ...
    __parse_acc_data(
        chan, &status, ACC_FORMAT_INTERLEAVED0, &hidReport[RPT_ACC_OFFSET], 0
    );
    __parse_dpdex_data(
        chan, (WPADStatusEx **)&rxBuffer, RPT3E_DPD0, &hidReport[RPT3E_DPD0_OFFSET], 0
    );
    __parse_dpdex_data(
        chan, (WPADStatusEx **)&rxBuffer, RPT3E_DPD1, &hidReport[RPT3E_DPD1_OFFSET], 0
    );
...
static void __a1_3f_data_type(u8 chan, byte_t *hidReport, void *rxBuffer) {
    ...
    __parse_btn_data(p_wpd, status, BTN_FORMAT_INTERLEAVED, hidReport, 0);
    __parse_acc_data(
        chan, &status, ACC_FORMAT_INTERLEAVED1, &hidReport[RPT_ACC_OFFSET], 0
    );
    __parse_dpdex_data(
        chan, (WPADStatusEx **)&rxBuffer, RPT3F_DPD2, &hidReport[RPT3F_DPD2_OFFSET], 0
    );
    __parse_dpdex_data(
        chan, (WPADStatusEx **)&rxBuffer, RPT3F_DPD3, &hidReport[RPT3F_DPD3_OFFSET], 0
    );
...
static void __a1_unused_report(
    u8 chan __attribute__((unused)),
