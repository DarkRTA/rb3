...
CampaignCareerLeaderboardPanel::CampaignCareerLeaderboardPanel()
    : mCampaignCareerLeaderboardProvider(0), mScoreType(kScoreBand), mUseDLC(0) {}
...
void CampaignCareerLeaderboardPanel::ResultFailure() { HandleType(lb_failure_msg); }
...
void CampaignCareerLeaderboardPanel::Load() {
    ...
    mCampaignCareerLeaderboardProvider =
        new PlayerCampaignCareerLeaderboard(pProfile, this, mScoreType, mUseDLC);
}
...
Symbol CampaignCareerLeaderboardPanel::GetScoreType() const {
    if (mScoreType == kNumScoreTypes)
        return "record_label";
    else
        return ScoreTypeToSym(mScoreType);
}
...
BEGIN_HANDLERS(CampaignCareerLeaderboardPanel)
    ...
    HANDLE_ACTION(set_leaderboard_mode, {
        MILO_ASSERT(mCampaignCareerLeaderboardProvider, 0xAD);
        mCampaignCareerLeaderboardProvider->SetMode((Leaderboard::Mode)_msg->Int(2), true);
    })
    HANDLE_ACTION_IF(
        select_lb_row,
        mCampaignCareerLeaderboardProvider,
        mCampaignCareerLeaderboardProvider->OnSelectRow(
    ...
    HANDLE_ACTION_IF(
        ...
        _msg->Obj<UIList>(2)->SetSelected(
            mCampaignCareerLeaderboardProvider->GetStartingRow(), -1
        )
    )
