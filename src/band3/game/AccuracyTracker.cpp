...
AccuracyTracker::AccuracyTracker(TrackerSource* src, TrackerBandDisplay& banddisp, TrackerBroadcastDisplay& bcdisp) : Tracker(src, banddisp, bcdisp), mCurrentAccuracy(-1.0f) {
...
void AccuracyTracker::FirstFrame_(float){
...
void AccuracyTracker::UpdateGoalValueLabel(UILabel& label) const {
    label.SetTokenFmt(DataArrayPtr(tour_goal_accuracy_goal_format, (int)(mTargets.front() * 100.0f)));
}

void AccuracyTracker::UpdateCurrentValueLabel(UILabel& label) const {
    label.SetTokenFmt(DataArrayPtr(tour_goal_accuracy_result_format, (int)(mCurrentAccuracy * 100.0f)));
}
