...
GemPlayer::GemPlayer(BandUser* user, BeatMaster* bm, Band* band, int i, BandPerformer* perf) : Player(user, band, i, bm) {
...
void GemPlayer::SetFillLogic(FillLogic fl) {
    if (mBeatMatcher) mBeatMatcher->SetFillLogic(fl);
}

bool GemPlayer::IsAutoplay() const {
    return mBeatMatcher->IsAutoplay();
}
...
void GemPlayer::PrintAddHead(int mils, int idx, int pts, int ms_avg, int recent) {
    if (unk2c4) {
        ...
        PrintMsg(MakeString("%s (%dms) %d pts -- %dms avg  -- %d recent", id, mils, pts, ms_avg, recent));
    }
}
...
BEGIN_HANDLERS(GemPlayer)
    HANDLE_ACTION(disable_fills_deploy_band_energy, mBehavior->SetFillsDeployBandEnergy(false))
    HANDLE_ACTION(enable_fills_deploy_band_energy, mBehavior->SetFillsDeployBandEnergy(true))


    HANDLE_ACTION(win, unk_0x2cc ? unk_0x2cc->WinGame(_msg->Int(2)) : WinGame(_msg->Int(2)))
