...
const char* sHitTypeNames[] = {
    ...
    "High Green fret", "High Red fret", "High Yellow fret", "High Blue fret", "High Orange fret",
    "Kick", "Yellow Cymbal", "Blue Cymbal", "Green Cymbal",
    "Upstrum", "Downstrum", "Start", "Select"
};
...
void HitTracker::Hit(HitType ty){
    mHits[ty]++;
}

void HitTracker::Key(int key){
    mKeys[key]++;
}

void HitTracker::RGFret(int str, int fret){
    mRGFrets[str][fret]++;
}

void HitTracker::RGStrum(int str){
    mRGStrums[str]++;
}
