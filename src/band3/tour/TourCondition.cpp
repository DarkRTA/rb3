...
void TourCondition::Init(const DataArray *arr) { mConds = arr; }

bool TourCondition::IsMet(const TourProgress &tp) const {
    if (mConds)
        return IsAndConditionMet(tp, mConds);
    else
        return true;
}
...
bool TourCondition::IsConditionMet(const TourProgress &tp, const DataArray *arr) const {
    ...
    if (sym == sym_and)
        return IsAndConditionMet(tp, arr);
    else if (sym == sym_or)
        return IsOrConditionMet(tp, arr);
    else if (sym == sym_not)
        return IsNotConditionMet(tp, arr);
        ...
        return IsComparisonConditionMet(tp.GetTourProperties(), arr);
}

bool TourCondition::IsComparisonConditionMet(
    const TourPropertyCollection &coll, const DataArray *arr
) const {
    ...
    Symbol sym = arr->Sym(0);
    if (sym == sym_greater)
        return IsGreaterConditionMet(coll, arr);
    else if (sym == sym_less)
        return IsLessConditionMet(coll, arr);
    else if (sym == sym_greater_equal)
        return IsGreaterEqualConditionMet(coll, arr);
    else if (sym == sym_less_equal)
        return IsLessEqualConditionMet(coll, arr);
    else if (sym == sym_equal)
        return IsEqualConditionMet(coll, arr);
    else if (sym == sym_not_equal)
        return IsNotEqualConditionMet(coll, arr);
    else {
...
bool TourCondition::IsAndConditionMet(const TourProgress &tp, const DataArray *arr) const {
    for (int i = 1; i < arr->Size(); i++) {
        if (!IsConditionMet(tp, arr->Array(i)))
            return false;
    }
    return true;
}
...
bool TourCondition::IsNotConditionMet(const TourProgress &tp, const DataArray *i_pArray)
    const {
    MILO_ASSERT(i_pArray->Size() == 2, 0xAF);
    return !IsConditionMet(tp, i_pArray->Array(1));
}

bool TourCondition::IsGreaterConditionMet(
    const TourPropertyCollection &tp, const DataArray *i_pArray
) const {
...
bool TourCondition::IsLessConditionMet(
    const TourPropertyCollection &tp, const DataArray *i_pArray
) const {
...
bool TourCondition::IsGreaterEqualConditionMet(
    const TourPropertyCollection &tp, const DataArray *i_pArray
) const {
...
bool TourCondition::IsLessEqualConditionMet(
    const TourPropertyCollection &tp, const DataArray *i_pArray
) const {
...
bool TourCondition::IsEqualConditionMet(
    const TourPropertyCollection &tp, const DataArray *i_pArray
) const {
...
bool TourCondition::IsNotEqualConditionMet(
    const TourPropertyCollection &tp, const DataArray *i_pArray
) const {
