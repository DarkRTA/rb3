...
namespace Quazal {
    ...
    qResult::qResult()
        : m_iReturnCode(0x80010001), m_cszFilename(__FILE__), m_iLineNumber(0x14) {}

    qResult::qResult(const int &i)
        : m_iReturnCode(i), m_cszFilename(__FILE__), m_iLineNumber(0x1B) {}

    bool qResult::Equals(const int &i) const { return m_iReturnCode == i; }

    bool qResult::Equals(const bool &b) const { return (m_iReturnCode >> 31 == 0) == b; }

    qResult::operator bool() const { return m_iReturnCode >> 31 == 0; }
