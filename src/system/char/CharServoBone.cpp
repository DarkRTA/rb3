...
CharServoBone::CharServoBone() : mPelvis(0), mFacingRotDelta(0), mFacingPosDelta(0), mFacingRot(0), mFacingPos(0), mMoveSelf(0), mDeltaChanged(0), mRegulate(this) {
    ...
    if(!mMeshes.empty()){
        ...
        if(mFacingPosDelta){
            if(!mMoveSelf){
                ...
                for(ObjDirItr<CharBone> it(CharBoneDir::FindResourceFromClipType(mClipType), false); it != nullptr; ++it){
                    if(it->BakeOutAsTopLevel()){
                        String str(it->Name());
                        if(str.find(".cb") != String::npos){
                            str = str.substr(0, str.length() - 3);
                        }
                        RndTransformable* boneTrans = CharUtlFindBoneTrans(str.c_str(), Dir());
                        if(mDeltaChanged){
...
void CharServoBone::DoRegulate(Waypoint* waypoint, CharClipDriver* driver, float f3, float f4){
...
void CharServoBone::PollDeps(std::list<Hmx::Object*>& changedBy, std::list<Hmx::Object*>& change){
    StuffMeshes(change);
}
