...
bool FileMergerSort(const FileMerger::Merger* m1, const FileMerger::Merger* m2){
    ...
    if(aData.unk4 && bData.unk4){
        ...
        bool females = strstr(m1->loading.c_str(), "female") && strstr(m2->loading.c_str(), "female");
        if(females)
    ...
    else return aData.priority < bData.priority;
}
...
FileMerger::Merger* FileMergerOrganizer::FrontInactiveMerger(OrganizedFileMerger* ofm){
    ...
    if(ofm->merger->mCurLoader) it++;
    if(it == ofm->merger->mFilesPending.end()) return nullptr;
    else return *it;
}
...
void FileMergerOrganizer::CheckDone(){
    ...
    for(std::list<OrganizedFileMerger>::iterator it = unk20.begin(); it != unk20.end(); ++it){
...
void FileMergerOrganizer::AddFileMerger(FileMerger* fm){
    if(!gOrganizing) fm->LaunchNextLoader();
    else {
...
void FileMergerOrganizer::FailedLoading(Loader* l){
    ...
    for(std::list<OrganizedFileMerger>::iterator it = unk20.begin(); it != unk20.end(); ++it){
        org = &*it;
        if(org->merger->mCurLoader && org->merger->mCurLoader == l) break;
    }
