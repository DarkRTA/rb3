...
/** "A CharBone container, acts as a resource file, storing skeleton and DOF for
 * particular contexts" */
...
public:
    ...
    NEW_OBJ(CharBoneDir)
    static void Register() { REGISTER_OBJ_FACTORY(CharBoneDir) }

    /** "Used to limit travel.  Moves [targets] bones so that the average of the [average]
     * bones will be at (0,0,0) at the start of the clip. If slide is true evaluates the
     * [average] bones and the start of the clip and end of the clip, and recenters
     * [targets] smoothly between those." */
