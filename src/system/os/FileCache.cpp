...
void FileCache::StartSet(int iii){
    ...
    for(int i = 0; i < mEntries.size(); i++){
        ...
        if((!curEntry->CheckSize() || curEntry->Fail()) && !curEntry->mLoader && !curEntry->mRefCount){
            delete curEntry;
...
void FileCache::Add(const FilePath& fp1, int iii, const FilePath& fp2){
    ...
    FilePath fp30;
    if(fp2.empty()) fp30 = file;
    else fp30.SetRoot(DirLoader::CachedPath(fp2.c_str(), 0));
...
void FileCache::DumpOverSize(int iii){
    ...
    while(i2 > iii){
        ...
        for(int i = 0; i < mEntries.size(); i++){
            FileCacheEntry* curEntry = mEntries[i];
            if(curEntry->CheckSize() && !curEntry->mLoader && !curEntry->mRefCount && (u9 == -1 || curEntry->mPriority < i8 || 
...
bool FileCache::DoneCaching(){
    for(int i = 0; i < mEntries.size(); i++){
        if(!mEntries[i]->ReadDone(true)) return false;
    }
    return true;
}