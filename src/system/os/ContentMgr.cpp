...
const char *gContentStateName[11] = {
    "kUnmounted",       "kNeedsMounting", "kMounting",    "kUnmounting",
    "kMounted",         "kAlwaysMounted", "kNeedsBackup", "kBackingUp",
    "kContentDeleting", "kDeleted",       "kFailed"
};
    ...
    if (mState == 4) {
        if (mLoader) {
            ...
            if (mCallbackFiles.empty()) {
                for (std::list<Content *>::iterator it = mContents.begin();
                     it != mContents.end();
                     ++it) {
                    ...
                    for (std::list<Callback *>::iterator it2 = mCallbacks.begin();
                         it2 != mCallbacks.end();
                         ++it2) {
                        mCallback = *it2;
                        ...
                        if ((*it2)->ContentDir()) {
                            String pathStr(
                                FileMakePath((*it)->Root(), (*it2)->ContentDir(), 0)
                            );
                            String makeStr(
                                MakeString("%s/%s", pathStr, (*it2)->ContentPattern())
                            );
                        ...
                        if ((*it2)->HasContentAltDirs()) {
                            ...
                            for (std::vector<String>::iterator dirIt = altDirs->begin();
                                 dirIt != altDirs->end();
                                 ++dirIt) {
                                DataArray *cfg = SystemConfig("net", "game");
                                const char *codeStr = cfg->FindStr("code");
                                if (mName.contains(MakeString("%s/", codeStr))
                                    && !mName.contains("_song")) {
                                    AddCallbackFile("dlc/content/songs", "songs.dta");
                                } else {
                                    String pathStr(
                                        FileMakePath((*it)->Root(), dirIt->c_str(), 0)
                                    );
