...
int MassChannelMapping::ChannelBitfield(int) const {
    int mask = 0;
    for(std::list<int>::const_iterator it = mChannels.begin(); it != mChannels.end(); ++it){
        mask |= (1 << *it);
    }
    return mask;
}

MultiChannelMapping::MultiChannelMapping(Submix* submix, const char* cc, std::vector<int>& vec){
    if(!submix) MILO_FAIL("submix '%s' is not defined", cc);
    for(int i = 0; i < submix->GetNumSlots(); i++){
        ...
        if(vec.size() != 0){
            for(std::list<int>::iterator it = curList.begin(); it != curList.end(); ++it){
                if(*it >= vec.size()){
                    MILO_WARN("MIDI authoring error: submix %s refers to channel %d when only channels 0-%d are available", cc, *it, vec.size() - 1);
                }
                else {
                    int vecInt = vec[*it];
                    int& listInt = *it;
...
void MultiChannelMapping::FillChannelList(std::list<int>& chanList) const {
    for(int i = 0; i < mChannelsPerSlot.size(); i++){
        chanList.insert(chanList.end(), mChannelsPerSlot[i].begin(), mChannelsPerSlot[i].end());
    }
}

void MultiChannelMapping::FillChannelList(std::list<int>& chanList, int laneNum) const {
    if(laneNum == -1) FillChannelList(chanList);
    else {
        ...
        chanList.insert(chanList.end(), mChannelsPerSlot[laneNum].begin(), mChannelsPerSlot[laneNum].end());
    }
}
...
int MultiChannelMapping::ChannelBitfield(int slot) const {
    ...
    if(slot >= mChannelsPerSlot.size()){
        MILO_FAIL("asking about lane %d but drums only have %d channels of audio\n", slot + 1, mChannelsPerSlot.size());
    }
    int mask = 0;
    for(std::list<int>::const_iterator it = mChannelsPerSlot[slot].begin(); it != mChannelsPerSlot[slot].end(); ++it){
        mask |= (1 << *it);
    }
    return mask;
}