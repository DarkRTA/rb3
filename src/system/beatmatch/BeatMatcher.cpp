...
BeatMatcher::BeatMatcher(const UserGuid& u, int i1, int i2, Symbol s, SongData* data, SongInfo& info, DataArray* arr, BeatMaster* bm) :
    mWaitingForAudio(1), mUserGuid(u), unk1c(i1), unk20(i2), mControllerType(s), mSongData(data), mCfg(arr), mSink(0), mAudio(bm->GetMasterAudio()), mController(0), mMercurySwitchFilter(0), mWatcher(0),
...
void BeatMatcher::Poll(float f){
    SetNow(f);
    if(mController) mController->Poll();
...
void BeatMatcher::Jump(float f){
    if(mNow != f){
        ...
        unka4 = -1;
        mWatcher->Jump(f);
        if(mMercurySwitchFilter) mMercurySwitchFilter->Reset();
        mWaitingForAudio = true;
    }
}

void BeatMatcher::Restart(){
    mWatcher->Restart();
}
...
void BeatMatcher::FretButtonDown(int i1, int i2){
    if(!unkad){
        ...
        if(!gemList->Empty()){
            if(TheBeatMatchOutput.IsActive()){
                TheBeatMatchOutput.Print(MakeString("(%2d%10.1f DOWN\t%d\t%d)\n", TheBeatMatchPlayback.GetPlaybackNum(this), mNow + unk98, i1, i2));
            }
...
void BeatMatcher::FretButtonUp(int i1){
    if(!unkad){
        ...
        if(!gemList->Empty()){
            if(TheBeatMatchOutput.IsActive()){
                TheBeatMatchOutput.Print(MakeString("(%2d%10.1f UP\t%d)\n", TheBeatMatchPlayback.GetPlaybackNum(this), mNow + unk98, i1));
            }
...
bool BeatMatcher::Swing(int i1, bool b2, bool b3, bool b4, bool b5, GemHitFlags flags){
    if(!unkad){
        if(!mSongData->GetGemList(mCurTrack)->Empty()){
            ...
            if(bucket != 0 && (mNow - f1 <= 25.0f) && unk8c > 1 && bucket == 1) ret = true;
            if(b5 && mNow <= unk80 + 52.0f) ret = true;
            if(b2){
                if(mNow <= unk80 + 150.0f && unk88 == b3 && mNow <= unk84 + 50.0f) ret = true;
                unk88 = b3;
            }
            MILO_ASSERT(mSink, 0x139);
            mSink->Swing(mCurTrack, i1, mNow, b3, b4);
            if(TheBeatMatchOutput.IsActive()){
                TheBeatMatchOutput.Print(MakeString("(%2d%10.1f SWING\t%d %d)\n", TheBeatMatchPlayback.GetPlaybackNum(this), i1, b2));
            }
            if(!b3) flags = (GemHitFlags)(flags | kGemHitFlagUpstrum);
            ...
            if(watcherSwing) return watcherSwing;
            if(TheBeatMatchOutput.IsActive()){
                TheBeatMatchOutput.Print(MakeString("(%2d%10.1f NOSWING\t%d %d)\n", TheBeatMatchPlayback.GetPlaybackNum(this), i1, b2));
                return false;
            }
        }
    }
