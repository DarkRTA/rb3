...
void FixedSizeSaveable::LoadFixedString(
    FixedSizeSaveableStream &fixedStream, String &str
) {
    int start = fixedStream.Tell();
    fixedStream >> str;
...
void FixedSizeSaveable::SaveStd(
    FixedSizeSaveableStream &fs, const std::vector<Symbol> &vec, int i
) {
    int size = vec.size();
    fs << size;
    ...
    if (i > size)
        PadStream(fs, (i - size) * 4);
}

void FixedSizeSaveable::LoadStd(
    FixedSizeSaveableStream &fs, std::vector<Symbol> &vec, int i
) {
    int size;
    fs >> size;
    vec.resize(size);
    ...
    if (i > size)
        DepadStream(fs, (i - size) * 4);
}

void FixedSizeSaveable::SaveStd(
    FixedSizeSaveableStream &fs, const std::list<Symbol> &list, int i
) {
    int size = list.size();
    fs << size;
...
void FixedSizeSaveable::LoadStd(
    FixedSizeSaveableStream &fs, std::list<Symbol> &list, int i
) {
    int size;
    fs >> size;
...
void FixedSizeSaveable::SaveStd(
    FixedSizeSaveableStream &fs, const std::set<Symbol> &set, int i
) {
    int size = set.size();
    fs << size;
...
void FixedSizeSaveable::LoadStd(
    FixedSizeSaveableStream &fs, std::set<Symbol> &set, int i
) {
    int size;
    fs >> size;
    for (int idx = 0; idx < size; idx++) {
