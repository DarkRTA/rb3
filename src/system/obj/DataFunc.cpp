...
DEF_DATA_FUNC(DataAddEq){
    ...
    if(array->Type(1) == kDataProperty){
        ...
        gDataThis->SetProperty(array->FastArray(1), ret);
    }
...
DEF_DATA_FUNC(DataSize) {
    if (array->Type(1) == kDataProperty) {
        ...
        return gDataThis->PropertySize(array->FastArray(1)); // TODO figure out what this actually is
    }
...
DEF_DATA_FUNC(DataHandleType) {
    for (int i = 1; i < array->Size(); i++) {
        ...
        if (n.Type() == kDataObject) o = n.FastObj();
...
DEF_DATA_FUNC(DataExport) {
    ...
    DataNode& n = a->Evaluate(0);
    Hmx::Object* obj;
    if (n.Type() == kDataObject) obj = n.FastObj();
...
DataNode Quasiquote(const DataNode& node) {
    ...
    if(nodeType - 0x10 <= 1U){
        DataArray* nodeArr = node.FastArray();
        if(nodeType == kDataCommand && nodeArr->Type(0) == kDataSymbol){
            char* str = (char*)nodeArr->Node(0).FastStr();
