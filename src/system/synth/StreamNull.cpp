...
StreamNull::StreamNull(float f) : mFaders(nullptr) {
    ...
    for(std::vector<FaderGroup*>::iterator it = mChannelFaders.begin(); it != mChannelFaders.end(); ++it){
...
bool StreamNull::IsPlaying() const {
    return mTimer.mRawTimer.Running();
}

bool StreamNull::IsFinished() const {
    return !mTimer.mRawTimer.Running();
}
...
FaderGroup* StreamNull::ChannelFaders(int idx){
    return mChannelFaders[idx];
}