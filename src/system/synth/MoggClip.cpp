...
MoggClip::MoggClip()
    : mFilePath(), mVolume(0.0f), mLoop(0), mControllerVolume(0.0f), mStream(0),
      unk44(0.0f), unk48(0), unk4c(0), mFileLoader(0), mFaders(), mPanInfos() {
...
void MoggClip::Load(BinStream &bs) {
    ...
    PostLoad(bs);
    MILO_WARN(
        "%s:  Ogg files are not supported in RB3_Wii.", (const char *)FindPathName()
    );
}
...
void MoggClip::Play() {
    if (EnsureLoaded()) {
        ...
        mStream = dynamic_cast<StandardStream *>(
            TheSynth->NewBufStream(unk48, unk4c, "mogg", 0.0f, 0)
        );
        mFader->SetVal(0.0f);
        SetLoop(mLoop);
        SetVolume(mVolume);
        ...
        MILO_WARN("Mogg file not loaded: '%s'", mFilePath.c_str());
}
...
void MoggClip::SetPan(int chan, float pan) {
    ...
    if (mStream)
        mStream->SetPan(pinfo.channel, pinfo.panning);
}
...
void MoggClip::LoadFile(BinStream *bs) {
    ...
    if (!mFilePath.empty()) {
        ...
        mFileLoader = new FileLoader(
            mFilePath,
            FileLocalize(mFilePath.c_str(), 0),
            kLoadFront,
            0,
            false,
            true,
        ...
        if (!mFileLoader)
            MILO_WARN("Could not load mogg file '%s'", mFilePath.c_str());
    }
}
...
BEGIN_PROPSYNCS(MoggClip)
    SYNC_PROP_SET(file, mFilePath, SetFile(_val.Str()))
    SYNC_PROP_SET(volume, mVolume, SetVolume(_val.Float()))
    SYNC_PROP_SET(loop, mLoop, SetLoop(_val.Int() != 0))
    SYNC_PROP_SET(loop_start_sample, mLoopStart, SetLoopStart(_val.Int()))
    SYNC_PROP_SET(loop_end_sample, mLoopEnd, SetLoopEnd(_val.Int()))
END_PROPSYNCS
