...
SampleZone::SampleZone(Hmx::Object *obj)
    : mSample(obj, 0), mVolume(0.0f), mPan(0.0f), mCenterNote(0x24), mMinNote(0),
...
bool SampleZone::Includes(unsigned char uc1, unsigned char uc2) {
    bool ret = false;
    if (mMinNote <= uc1 && uc1 <= mMaxNote && mMinVel <= uc2 && uc2 <= mMaxVel)
        ret = true;
    return ret;
}