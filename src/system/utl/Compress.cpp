...
void *ZAlloc(void *i, unsigned int len, unsigned int ct) {
    return _MemAllocTemp(len * ct, 0);
}
void ZFree(void *a, void *b) { _MemFree(b); }

void DecompressMem(
    const void *in, int in_len, void *out, int &out_len, bool bits, const char *filename
) {
    z_stream s;
