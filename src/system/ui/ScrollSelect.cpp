...
bool ScrollSelect::CanScroll() const {
    bool ret = false;
    if(!mSelectToScroll || mSelectedAux != -1) ret = true;
    return ret;
}
...
DataNode ScrollSelect::Handle(DataArray* _msg, bool _warn){ 
    ...
    if(_warn) MILO_WARN("%s(%d): %s unhandled msg: %s", __FILE__, 0x85, PathName(dynamic_cast<Hmx::Object*>(this)), sym);
