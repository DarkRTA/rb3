...
DECOMP_FORCEACTIVE(UITransitionHandler, __FILE__, "0")

UITransitionHandler::UITransitionHandler(Hmx::Object *obj)
    : mInAnim(obj, 0), mOutAnim(obj, 0), mAnimationState(0), mChangePending(0), b3(0) {}
...
void UITransitionHandler::StartValueChange() {
    ...
    if (theState == 0) {
        ...
        if (b3)
            ...
            mOutAnim->Animate(
                mOutAnim->GetFrame(), mOutAnim->StartFrame(), mOutAnim->Units(), 0.0f, 0.0f
            );
            mAnimationState = 4;
        }
    }
...
void UITransitionHandler::ClearAnimationState() { mAnimationState = 0; }
