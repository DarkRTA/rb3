...
public:
    UIGridSubProvider() {}
    UIGridSubProvider(UIListProvider *lprov, UIGridProvider *gprov, int i)
        : mListProvider(lprov), mGridProvider(gprov), mIndex(i) {}
    ...
    virtual Symbol DataSymbol(int i) const {
        return mListProvider->DataSymbol(i + mIndex * mGridProvider->mWidth);
    }
    virtual int DataIndex(Symbol s) const {
        return mListProvider->DataIndex(s) % mGridProvider->mWidth;
    }
