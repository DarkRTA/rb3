...
public:
    ...
    /** Determine whether or not we have a PropKeys with the supplied target object and property.
     * @param [in] obj The target object for the keys.
     * @param [in] prop The property of the target object the keys should animate.
     * @returns True if such PropKeys exists in our collection, false if not.
     */
    bool HasKeys(Hmx::Object* obj, DataArray* prop);
    /** Get the position of the PropKeys collection containing a PropKeys with the supplied target and prop.
     * @param [in] obj The target object the PropKeys must have.
     * @param [in] prop The property that the PropKeys must be animating.
    ...
    /** Get the PropKeys with the supplied target and prop, and add a new keyframe with the supplied value/frame pair.
     * @param [in] obj The target object the PropKeys must have.
     * @param [in] prop The property that the PropKeys must be animating.
     * @param [in] frame The frame of the keyframe we want to modify.
     * @param [in] val The value of the keyframe we want to modify.
     * @param [in] unique If true, and a keyframe at the supplied frame exists, modify the value. Else, create a new keyframe.
     * @returns An iterator of the PropKeys collection with the corresponding PropKeys.
     */
    void SetKeyVal(Hmx::Object* obj, DataArray* prop, float frame, const DataNode& val, bool unique);
    /** Get the animation keys type of the PropKeys with the supplied target and prop.
     * @param [in] obj The target object the PropKeys must have.
     * @param [in] prop The property that the PropKeys must be animating.
     * @returns The found PropKeys' animation type. If the PropKeys doesn't exist, return kFloat.
     */
     ...
     * @param [in] obj The target object the PropKeys must have.
     * @param [in] prop The property that the PropKeys must be animating.
     * @returns The found PropKeys' interpolation type. If the PropKeys doesn't exist, return kStep.
     */
     ...
     * @param [in] obj The target object the PropKeys must have.
     * @param [in] prop The property that the PropKeys must be animating.
     * @returns The found PropKeys' handler. If the PropKeys doesn't exist, return a null Symbol.
     */
     ...
     * @returns The index of the supplied PropKeys frame, or -1 if the PropKeys doesn't exist.
     ...
     * Example usage: {$this index_from_frame targetObj (targetPropKey targetPropVal) 69.0}
     */
    DataNode OnGetIndexFromFrame(const DataArray* arr);
    /** Handler to get the frame at a PropKeys' index.
     * @param [in] arr The supplied DataArray.
     * @returns The frame of the supplied PropKeys index, or -1.0 if the PropKeys doesn't exist.
     * Expected DataArray contents: 
     *     Node 2: The target object of the PropKeys.
     *     Node 3: The property associated with the PropKeys (as a DataArray).
     *     Node 4: The index of the PropKeys to get.
     * Example usage: {$this frame_from_index targetObj (targetPropKey targetPropVal) 420}
     */
     ...
     * Example usage: {$this value_from_index targetObj (targetPropKey targetPropVal) 420 $valueVar}
     */
     ...
     *     Node 3: The property associated with the PropKeys (as a DataArray).
     *     Node 4: The frame of the PropKeys to search.
     * Example usage: {$this value_from_frame targetObj (targetPropKey targetPropVal) 69.0}
     */
