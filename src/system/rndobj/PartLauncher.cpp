...
BEGIN_PROPSYNCS(RndPartLauncher)
    ...
    SYNC_PROP_SET(override_delta_size, int(mPartOverride->mask >> 3 & 1), SetBit(8, _val.Int()))
    SYNC_PROP(delta_size, mPartOverride->deltaSize)
    SYNC_PROP_SET(override_start_color, int(mPartOverride->mask >> 4 & 1), SetBit(0x10, _val.Int()))
    SYNC_PROP(start_color, mPartOverride->startColor)
    SYNC_PROP(start_alpha, mPartOverride->startColor.alpha)
    SYNC_PROP_SET(override_mid_color, int(mPartOverride->mask >> 5 & 1), SetBit(0x20, _val.Int()))
    SYNC_PROP(mid_color, mPartOverride->midColor)
    SYNC_PROP(mid_alpha, mPartOverride->midColor.alpha)
    SYNC_PROP_SET(override_end_color, int(mPartOverride->mask >> 6 & 1), SetBit(0x40, _val.Int()))
    SYNC_PROP(end_color, mPartOverride->endColor)
    SYNC_PROP(end_alpha, mPartOverride->endColor.alpha)
    SYNC_PROP_SET(override_emit_direction, int(mPartOverride->mask >> 7 & 1), SetBit(0x80, _val.Int()))
    ...
    SYNC_PROP_SET(override_box_emitter, int(mPartOverride->mask >> 9 & 1), SetBit(0x200, _val.Int()))
    SYNC_PROP(box_extent_1, mPartOverride->box.mMin)
    SYNC_PROP(box_extent_2, mPartOverride->box.mMax)
    SYNC_PROP_SET(override_mesh_emitter, int(mPartOverride->mask >> 8 & 1), SetBit(0x100, _val.Int()))
