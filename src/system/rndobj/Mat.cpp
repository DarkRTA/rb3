...
MatShaderOptions::MatShaderOptions() : mTempMat(0) { pack = mTempMat | 0x12; }
...
MatPerfSettings::MatPerfSettings()
    : mRecvProjLights(0), mRecvPointCubeTex(0), mPS3ForceTrilinear(0) {}
...
RndMat::RndMat()
    : mColor(1.0f, 1.0f, 1.0f, 1.0f), mDiffuseTex(this), mAlphaThresh(0), mNextPass(this),
      mEmissiveMap(this), mRefractStrength(0.0f), mRefractNormalMap(this), mIntensify(0),
      mUseEnviron(1), mPreLit(0), mAlphaCut(0), mAlphaWrite(0), mCull(1), mPerPixelLit(0),
...
BEGIN_PROPSYNCS(RndMat)
    ...
    }
    {
        ...
        if (sym == _s) {
            if (_op == kPropSet)
                mPointLights = _val.Int();
            else
                _val = DataNode(mPointLights);
            return true;
        }
    }
    {
    ...
    }
    {
        ...
        if (sym == _s) {
            if (_op == kPropSet)
                mColorAdjust = _val.Int();
            else
                _val = DataNode(mColorAdjust);
            return true;
        }
    }
    SYNC_PROP_SET(
